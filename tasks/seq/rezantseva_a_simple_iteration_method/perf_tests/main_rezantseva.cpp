...
TEST(rezantseva_a_simple_iteration_method_seq, test_pipeline_run) {
  size_t size = 1000;
  auto [A, b] = rezantseva_a_simple_iteration_method_seq::createRandomMatrix(size);
  ...
  auto testTaskSequential =
      std::make_shared<rezantseva_a_simple_iteration_method_seq::SimpleIterationSequential>(taskDataSeq);
...
TEST(rezantseva_a_simple_iteration_method_seq, test_task_run) {
  size_t size = 1000;
  auto [A, b] = rezantseva_a_simple_iteration_method_seq::createRandomMatrix(size);
  std::vector<double> out(size, 0.0);
