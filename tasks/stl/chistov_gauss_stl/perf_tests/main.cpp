...
#include "stl/chistov_gauss_stl/include/ops_stl.hpp"
 ...
 TEST(SEQ, test_task_run) {
   ...
   // Create Task
   auto test_task_sequential = std::make_shared<chistov_gauss_stl::TestTaskSequential>(task_data_seq);
 ...
 TEST(OMP, test_task_run) {
   ...
   // Create Task
   auto test_task = std::make_shared<chistov_gauss_stl::TestTaskOpenMP>(task_data_omp);
...
TEST(STL, test_task_run) {
  ...
  task_data_omp->outputs_count.emplace_back(output_image.size());
   ...
   //Create Task
  auto test_task = std::make_shared<chistov_gauss_stl::TestTaskSTL>(task_data_omp);
...
TEST(COMPARE, test_task_run) {
  ...
  chistov_gauss_stl::TestTaskSequential image_filter_sequential(task_data_seq);
 ...
 //  Create Perf analyzer
  ...
  perf_analyzer->task_run(perf_attr, perf_results);
  ppc::core::Perf::print_perf_statistic(perf_results);
  chistov_gauss_stl_test::CompareImages(output_image_omp, output_image_seq, width, height);
}
