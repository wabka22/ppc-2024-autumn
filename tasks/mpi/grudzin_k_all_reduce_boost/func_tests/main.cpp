...
TEST(grudzin_k_all_reduce_boost_mpi, Test_Seq_and_Boost_Ver_17x23) {
  int rows = 17;
  int colums = 23;
  boost::mpi::communicator world;
  std::vector<int> mas(colums * rows);
  mas = grudzin_k_all_reduce_boost_mpi::GenMatrix(colums, rows);
  ...
  grudzin_k_all_reduce_boost_mpi::TestMPITaskBoostRealization TestMPITaskBoostRealization(taskDataPar);
  ...
  if (world.rank() == 0) {
    ...
    grudzin_k_all_reduce_boost_mpi::TestMPITaskSequential testMpiTaskSequential(taskDataSeq);
...
TEST(grudzin_k_all_reduce_boost_mpi, Test_Seq_and_Boost_Ver_16x32) {
  int rows = 16;
  int colums = 32;
  boost::mpi::communicator world;
  std::vector<int> mas(colums * rows);
...
TEST(grudzin_k_all_reduce_boost_mpi, Test_Seq_and_Boost_Ver_2000x2000) {
  int rows = 2000;
  int colums = 2000;
...
TEST(grudzin_k_all_reduce_boost_mpi, Test_Seq_and_Boost_Ver_1337) {
  int rows = 13;
  int colums = 37;
...
TEST(grudzin_k_all_reduce_boost_mpi, Test_Seq_and_Boost_Ver_20x30) {
  int rows = 20;
  int colums = 30;
...
TEST(grudzin_k_all_reduce_boost_mpi, Empty_Val) {
  const int rows = 0;
  const int colums = 0;
  ...
  if (world.rank() == 0) {
    ...
    grudzin_k_all_reduce_boost_mpi::TestMPITaskSequential testMpiTaskSequential(taskDataPar);
    grudzin_k_all_reduce_boost_mpi::TestMPITaskBoostRealization testMpiTaskMR(taskDataPar);
    ASSERT_EQ(testMpiTaskSequential.validation(), false);
    ASSERT_EQ(testMpiTaskMR.validation(), false);
  }
}

TEST(grudzin_k_all_reduce_boost_mpi, Forget_One_Size_Val) {
  const int rows = 10;
  const int colums = 10;
...
TEST(grudzin_k_all_reduce_boost_mpi, Ans_NEQ_Col_Val) {
  const int rows = 10;
  const int colums = 10;
