...
#include "tbb/chistov_gauss_tbb/include/ops_tbb.hpp"
 ...
 void RunImageFilterTest(std::shared_ptr<ppc::core::TaskData> task_data_tbb) {
   chistov_gauss_tbb::TestTaskTBB image_filter_sequential(task_data_tbb);
...
TEST(chistov_gauss_tbb, test_empty_image) {
  ...
  chistov_gauss_tbb::TestTaskTBB test_task_tbb(task_data_tbb);
  ASSERT_FALSE(test_task_tbb.validation());
}

TEST(chistov_gauss_tbb, test_invalid_pixel_values) {
...
TEST(chistov_gauss_tbb, test_invalid_output_size) {
  constexpr size_t width = 3;
  constexpr size_t height = 3;
...
TEST(chistov_gauss_tbb, test_too_small_image) {
...
TEST(chistov_gauss_tbb, test_small_image) {
  constexpr size_t width = 3;
  constexpr size_t height = 3;
...
std::vector<double> expected_image = {10.0, 20.0, 20.0, 32.5, 50.0, 42.5, 55.0, 80.0, 65.0};
  ...
  auto task_data_tbb = chistov_gauss_tbb_test::CreateTaskData(input_image, output_image, kernel, width, height);
  chistov_gauss_tbb_test::RunImageFilterTest(task_data_tbb);
  chistov_gauss_tbb_test::CompareImages(output_image, expected_image, width, height);
}

TEST(chistov_gauss_tbb, test_non_standard_kernel) {
  constexpr size_t width = 3;
  constexpr size_t height = 3;
...
TEST(chistov_gauss_tbb, test_different_width_and_height) {
...
TEST(chistov_gauss_tbb, test_zero_values_image) {
  constexpr size_t width = 5;
  constexpr size_t height = 5;
...
TEST(chistov_gauss_tbb, test_max_pixel_values_image) {
